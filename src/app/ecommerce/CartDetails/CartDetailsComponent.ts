import { Component, OnInit, OnDestroy, afterNextRender, effect, ChangeDetectorRef, inject, ChangeDetectionStrategy } from '@angular/core';
...
export class CartDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      const hasViewingParam = this.route.snapshot.queryParams['viewingUserEmail'];
  ...
  private loadCartDetails(email: string): void {
    ...
    this.cartService.updateCartNavbar(
      ...
      this.cartDetails.reduce((sum, item) => sum + (item.price || 0) * (item.amount || 0), 0)
    );
    ...
    this.cartDetailService
      ...
      .subscribe({
        next: (details) => {
          ...
          const itemCount = this.cartDetails.reduce((sum, item) => sum + (item.amount || 0), 0);
          const cartTotal = this.cartDetails.reduce((sum, item) => 
            sum + ((item.price || 0) * (item.amount || 0)), 0);
